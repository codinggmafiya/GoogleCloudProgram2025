...
echo "${BLUE}${BOLD_TEXT}🛠️  D Please Subscribe to codingg mafiya ...${RESET_FORMAT}"
...
if [ -z "$REGION" ]; then
  ...
  if [ -n "$ZONE" ]; then
  echo "${BLUE}${BOLD_TEXT}⚙️  Trying to derive region from the provided zone: $ZONE${RESET_FORMAT}"
    REGION="${ZONE%-*}"
...
echo "${BLUE}${BOLD_TEXT}⚙️  Configuring default compute region to $REGION...${RESET_FORMAT}"
gcloud config set compute/region $REGION
echo "${GREEN_TEXT}${BOLD_TEXT}👍 Default compute region configured.${RESET_FORMAT}"
echo

echo "${BLUE}${BOLD_TEXT}🛠️  Enabling necessary Google Cloud services. This might take a moment...${RESET_FORMAT}"
...
echo "${BLUE}${BOLD_TEXT}🛠️  Ensuring Cloud Build GCS bucket (gs://${PROJECT_ID}_cloudbuild) exists...${RESET_FORMAT}"
...
echo "${BLUE}${BOLD_TEXT}ℹ️  Using 'web/artifacts.json' directly for Cloud Deploy release.${RESET_FORMAT}"
...
echo "${BLUE}${BOLD_TEXT}🖼️  Listing Docker images in the Artifact Registry repository...${RESET_FORMAT}"
...
echo "${BLUE}${BOLD_TEXT}⚙️  Configuring default Cloud Deploy region to $REGION...${RESET_FORMAT}"
...
if [ "$test_rollout_succeeded" = true ]; then
  ...
  echo "${BLUE}${BOLD_TEXT}➡️ Promoting release 'web-app-001' to the 'staging' target...${RESET_FORMAT}"
  ...
  echo "${BLUE}${BOLD_TEXT}➡️ Promoting release 'web-app-001' to the 'prod' target (this will require approval)...${RESET_FORMAT}"
    gcloud beta deploy releases promote \
    --delivery-pipeline web-app \
    --release web-app-001 \
    --quiet
